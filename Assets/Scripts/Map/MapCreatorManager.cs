	...
	void loadMapFromXml() {
		...
		for (int i = 0; i < mapSize; i++) {
			...
			for (int j = 0; j < mapSize; j++) {
                ...
                Tile tile = ((GameObject)Instantiate(GM.Prefabs.BASE_TILE_PREFAB, new Vector3(i - Mathf.Floor(mapSize / 2), tileHeight, -j + Mathf.Floor(mapSize / 2)), Quaternion.Euler(new Vector3()))).GetComponent<Tile>();
				tile.transform.parent = mapTransform;
				tile.gridPosition = new Vector2(i, j);
